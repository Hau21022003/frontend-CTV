...
const ReportPage: React.FC = () => {
  ...
  // const [years, setYears] = useState<string[]>([]); // years should default to an empty array
  ...
  //     setChoosed({ key: temp.id.toString(), name: temp.name });
  //     choosed.key = temp.id.toString();
  //     choosed.name = temp.name;
  ...
  //       `report/all-years?deptId=${choosed.key}`
  //     );
  ...
  const handleClear = (setter: any, disable: boolean) => {
    if (!disable) {
      setter({ key: "", name: "" });
      setIsDisableAddNewButton(false);
      setChoosed({ key: "", name: "" });
  ...
  return (
    <Layout>
      ...
      >
        ...
        >
          <div className="py-2 px-6 bg-white flex items-center justify-between shadow-md rounded-lg h-12">
            <Typography
              component="div"
              sx={{
                fontWeight: 700,
          ...
          >
            <ReportTable refresh={refreshTable} year={year} />
          </Box>
        </Box>
